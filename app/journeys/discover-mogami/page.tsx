    ...
    <section className="mogami-themes wrap" aria-label="最上を知る旅のテーマ">
      {mogamiJourneyThemes.map(theme => {
        ...
        return <article className={`mogami-theme mogami-theme-${theme.number}`} id={theme.slug} key={theme.id}>
          <a className="mogami-theme-image" href={trunkHref} aria-label={`${theme.titleJa}の写真トランク（準備中）`}><Image src={theme.image} alt={`${theme.titleJa}を伝える最上の風景`} fill sizes="(max-width: 700px) calc(100vw - 42px), 72vw" /></a>
