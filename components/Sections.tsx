...
export function PartnersDirectory() {
  ...
  return <main className="partners-page">
    <section className="wrap partners-directory">
      ...
      <Label>PARTNERS</Label>
      <header className="partners-heading">
        <div><h1>最上の旅を、ともにつくる。</h1><p lang="en">Creating journeys together in Mogami.</p></div>
        <div className="partners-intro"><p>宿泊、移動、地域での体験。<br />MOYA Travelは、最上をよく知る地域のパートナーとともに、<br />この土地ならではの旅をつくります。</p><p lang="en">Stays, transportation, and local experiences.<br />MOYA Travel works with trusted local partners<br />to create journeys rooted in the landscapes and communities of Mogami.</p></div>
      </header>
      <div className="partner-categories">{partnerCategories.map(category => {
        const categoryPartners = partners.filter(partner => partner.category === category);
        if (categoryPartners.length === 0) return null;
        const featured = categoryPartners.filter(partner => partner.image);
        const textOnly = categoryPartners.filter(partner => !partner.image);
